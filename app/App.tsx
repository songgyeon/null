...
const demoBucket = (t?:string) => {
  ...
  if(/스트레스|힘들|짜증|지쳐|지친|피곤|빡세/.test(x))return 'stress';
...
const DEMO_LINES:Record<string,Record<string,string[][]>> = {
  ...
  minhyun:{
    med:[['누가 발라줬어요?'],['보여줘요.']],
    meal:[['혼자요?'],['{it}요?','저는 아직 안 먹었는데.']],
    met:[['어디서요.'],['기억나요?','저는 나는데.']],
    nap:[['꿈에 선생님 나왔어요','무슨 내용인지 알려줄까요?']],
    friend:[['저는 선생님이랑 어울리는 게 좋아요']],
    stress:[['옥상 갈래요?','아무도 안 오는 곳 아는데']],
