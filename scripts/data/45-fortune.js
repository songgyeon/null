...
/* ── ✧ NULL 위한 오늘의 운세 ✧ ──
   ...
   who/place/find 세 빈칸은 유저가 직접 쓴다. 시스템이 정하는 것은 keyword 하나뿐이다.
   FILL을 눌러 세 칸이 모두 확정된 뒤에만 키워드를 공개한다. 모델 요청에는 공개된
   오늘의 keywordId 하나만 보낼 수 있고, 유저가 쓴 세 값은 기기 밖으로 나가지 않는다. */
...
/* 게임 0일차는 이름·등록·첫 만남의 날이다. Luck은 그 흐름을 덮지 않고
   실제 1일차부터 열린다. 시간표는 이 규칙과 별개로 0일차에도 유지된다. */
const fortuneAvailableForGameDay=day=>Number.isFinite(Number(day))&&Math.floor(Number(day))>=1;
/* 실제 진행값을 바꾸지 않는 UI 확인용 진입점. 쿼리를 명시한 탭에서만 켜지고
   localStorage를 읽거나 쓰지 않는다. */
const fortunePreviewRequested=()=>{try{
  return typeof location!=="undefined"
    &&new URLSearchParams(location.search).get("preview")==="luck";
...
const FORTUNE_INPUT_MAX={who:12,place:16,find:20};
const cleanFortuneInput=(key,value)=>String(value==null?"":value)
  .replace(/[\r\n\t]+/g," ").replace(/\s+/g," ").trim()
  .slice(0,FORTUNE_INPUT_MAX[key]||20);
...
const fortuneRecordOk=r=>!!r&&typeof r==="object"
  ...
  &&["who","place","find"].every(k=>typeof r[k]==="string"
    &&r[k].length<=(FORTUNE_INPUT_MAX[k]||20))
  &&!!FORTUNE_KEYWORD_BY_ID[r.keywordId]
  &&typeof r.revealed==="boolean"&&typeof r.seen==="boolean"
  &&(!r.revealed||["who","place","find"].every(k=>cleanFortuneInput(k,r[k])));
...
const fillFortuneForToday=(now,record,values)=>{
  ...
  const saved=fortuneRecordForDay(s,day);
  if(!saved)return current;
  const who=cleanFortuneInput("who",values&&values.who);
  const place=cleanFortuneInput("place",values&&values.place);
  const find=cleanFortuneInput("find",values&&values.find);
  if(!who||!place||!find)return saved;
  const next={...saved,who,place,find,revealed:true};
  if(!replaceFortuneRecord(s,next))return current;
  saveFortuneState(s);
  return next;
};
const revealFortuneForToday=(now,record,values)=>fillFortuneForToday(now,record,values);
