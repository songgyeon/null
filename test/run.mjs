...
/* ── 안이 비치면 안 된다 ──
   ...
   나갔다(플레이 기록 검수). 원인은 첫 「{」부터 문자열 끝까지
   ...
   몇 시인지는 안 준다 — 분을 주면 「7시 42분이네요」가 나온다. 때만 준다 */
{
  ...
  eq('때가 접속 상태와 안 어긋난다', (() => {
    ...
    for (let h = 0; h < 24; h++) {
      ...
      if (w === '새벽' && h >= 8) bad.push(h);        // 재언이 출근(8시)하기 전이어야
...
/* ── 내부 UI 복구 경계 ──
   오프닝의 배경·CD만 새 원화를 쓴다(등록 프로필은 CSS로 갔다). 나머지 화면은 fd84219의 원래
   메신저 DOM/CSS를 유지한다. 74b195c부터 붙은 ref 합성 UI도 복구 대상이다. */
{
  ...
  /* 프로필 판은 그림에서 CSS로 갔다. 칸마다 webp를 깔던 때는 이 자리에서
     ...
     그림이 없다 — 좌표와 cqw만 남았고 그건 아래의 「프로필 판을 CSS로
     그린다」가 본다 */
}
...
/* ── ④ 강현의 옛 일기 — 병원 옥상 ──
   강현을 실제로 만난 판에서만(오프닝이든 편의점 첫 만남이든), 「저 알죠」 세 줄이 앉은 뒤 유저가 처음
   무언가를 입력한 그 순간. 앞면이 천천히 뜨고 뒷면으로 천천히 넘어간다. */
{
  ...
  /* 이 값은 가변부(buildFlash)로 서버에 간다 — 계약은 그대로다: 이야기 상태에 섞이지 않는다 */
   ...
   있는 사람이 옆방에서 말을 하면 그 점이 거짓말이 된다 */
{
  ...
  /* 재언 1~4:30, 강현 4:30~8 — 둘을 합치면 1~8시가 조용하다 */
  eq('한쪽만 자도 관전은 안 만든다',
  ...
  /* ── 자는 사람을 셋으로 세지 않는다 ──
     ...
     새벽 두 시엔 재언만 자고 강현은 네 시 반까지 깨 있다 — 목록에 「안 자는 중」이
   ...
   첫인사를 보낸다 — 그래서 첫 화면에서 이미 둘의 시간대가 갈린다. */
{
  ...
  /* 첫 자리 네 곳(지도에 있는 자리)에는 전부 그 사람이 깔려야 한다. 빈 방으로 시작해서
...
// 방아쇠 다섯 개를 다 적어둬야 한다. 하나만 걸면 다른 쪽은 영영 안 열린다
...
/* ── F. 시계 통일 — 번역기는 하나다 ──
   ...
   항등이라 아무것도 안 변하고, 스피드 모드에서는 앵커(켠 그 시각)에서
...
/* 안쪽 여덟은 종이다 — 일기 넉 장, 상담 기록 둘, 그리고 계정 둘.
...
/* 한자가 한글에 붙어 있으면 단어 안에 낀 것이다. 지우면 「生수」가 「수」로,
   ...
   깨진 말풍선이 셋 나왔다(플레이 기록 검수). 한 줄 없어지는 것은
   ...
   나가는 거예요」라고 진행형으로 말했다 — 나가는 중이 아니라 나간 뒤인데. */
{
  ...
  /* 「나갔다」만 적어두니 모델이 저는 거기 없었던 것으로 읽고, 같이 있었잖냐고
     하자 없는 말을 지어내 우겼다(플레이 기록 검수). 같이 있었다는 것부터
   ...
   없는 말투로 연기한다 — 데모(각본)와 라이브(모델)가 다른 사람이 된다. */
{
  ...
  /* 재언은 문구집에서 -습니다/-습니까를 안 쓴다. 예외는 처음 말을 거는 두 줄뿐이다 —
     아직 아는 사이가 아닐 때만 허용된 어미라서, 여기가 늘어나면 규칙이 무너진 것이다. */
   ...
   데이터가 먼저, 화면 조각이 다음, 앱이 마지막. 하나라도 어긋나면 화면이 하얗다. */
{
     ...
     보이고 사람은 안 고쳐졌다고 한다. 모든 파일이 같은 번호여야 한다 */
  {
    ...
    eq('판 번호가 지금 내용의 것이다',
      [v[0][1], seal.digest('hex').slice(0, 12)], ['300', '277a567dc483']);
...
/* 자리로 들어가는 길은 처음엔 둘이었다 — 인물의 초대, 그리고 지도에서 내가 고르는 것.
...
{
  /* 자리는 icon으로 센다 — 체육관에 배경 사진이 없던 때 bg로 세면 빠졌다 */
  ...
  /* 아홉 자리가 두 자리에서 다 닿아야 한다 — 안 닿으면 영영 못 가는 자리가 생긴다 */
  {
...
/* ══════════ 공통 계약 ══════════
   ...
   이 시험은 배선(뒤 단계) 전에 먼저 썼다: 계약이 계약대로 도는지를
   배선 전에 굳혀두는 것이 요점이었다. 지금은 worker.js가 이 함수들을 쓴다. */
{
   ...
   프롬프트**를 들여다본다. */
{
  ...
  /* ── 물건의 정체성은 (수신자, 종류)다 ──
     ...
     막는다(scripts/game.js: `have.includes(gift.key)` — 수신자별로만).
