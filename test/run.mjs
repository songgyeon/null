   ...
   indexOf 필터가 세 갈래를 다 삼킨다 — 한 번 그랬다 */
{
  ...
  /* 이 다섯 줄이 골목을 설명하는 유일한 자리다. 「맞담했다」만 던지면
     골목을 모르는 플레이어는 학생이랑 담배 피우고 다니는 교생이 된다.
     맞담을 꺼낼 거면 유저가 무슨 말을 했고 나중에 어떻게 했는지가
     같이 나와야 한다 — 그래야 잘못한 쪽이 누구인지 읽힌다. */
  {
    const open = corpus.proactive.minhyun.find(p => p.when === '첫 만남').lines[0] || [];
    eq('민현이 골목을 통째로 꺼낸다', open.length, 5);
    eq('맞담을 꺼내면 책임도 같이 나온다',
      !/맞담/.test(open.join(' ')) || (/책임/.test(open.join(' ')) && /금연/.test(open.join(' '))), true);
    /* 끝은 확인이 아니라 청구서다. 골목에서 유저가 한 말을 그대로 돌려준다 */
    eq('청구서로 끝난다', open[open.length - 1], '그래서 책임은 어떻게 질 건데요?');
    eq('민현의 첫 마디도 존댓말이다', open.filter(t => /(는데요|라면서요|건데요|죠\?)/.test(t)).length >= 3, true);
  }
