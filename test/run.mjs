   ...
   indexOf 필터가 세 갈래를 다 삼킨다 — 한 번 그랬다 */
{
  ...
  /* 협박을 첫 줄로 두면 플레이어는 골목을 아직 모른 채로 학생이랑 맞담한
     교생이 된다. 실제로 그 장면에서 유저는 교복을 보자마자 담배를 밟아 껐다.
     확인이 먼저 오고, 답을 들은 뒤에야 협박이 농담으로 읽힌다. */
  eq('민현은 확인으로 문을 연다',
    (corpus.proactive.minhyun.find(p => p.when === '첫 만남').lines[0] || []).join(' / '),
    '선생님. / 저 기억나죠?');
  eq('첫 줄에 맞담 협박이 없다',
    (corpus.proactive.minhyun.find(p => p.when === '첫 만남').lines[0] || [])
      .some(t => /맞담|소문/.test(t)), false);
  /* 다만 그 협박이 사라진 건 아니다 — 인물 설정과 말투 예시에 그대로 있어서
     대화 중에 나온다. 첫 줄에서만 뺀 것이다. */
  eq('협박은 프롬프트에 남아 있다',
    /맞담했다고 소문낼 거예요/.test(workerSrc) && /소문낼 거예요/.test(workerSrc), true);
