   ...
   모진 것은 다르다. 어색함이 어디를 향하는지를 못 박는다. */
{
  ...
  eq('선물 앞의 어색함은 자기 쪽으로 향한다', g.includes('어색함은 네 쪽으로 향한다'), true);
  eq('준 물건을 깎아내리지 못하게 막는다',
    ['물건을 깎아내리지 않는다', '딴 데 쓰겠다고 하지 않는다',
     '가져가라고 하지 않는다', '"장난이었어요"로 수습하지 않는다',
     '무뚝뚝한 것과 모진 것은 다르다'].filter(s => !g.includes(s)), []);
  /* 「딴소리를 해도 된다」가 라면 받침의 출처였다 */
  eq('딴소리 허가는 걷었다', /딴소리를 해도 된다/.test(g), false);
}
...
/* 선물에만 선을 그으면 선물에서만 안 모질다. 매 방에 들어가는 말버릇에
   같은 선을 한 번 더 긋는다 — 유저가 한 것을 시시하게 만들지 않는다 */
eq('모진 말은 어느 방에서든 막는다',
  ['무뚝뚝한 것과 모진 것은 다르다', '유저가 한 것을 시시하게 만들지 않는다',
   '"장난이었어요"로 수습하지도 않는다']
    .filter(s => !workerSrc.includes(s)), []);
/* 민현 제 프롬프트는 「선을 조금 넘는다」인데 말버릇에는 「무례하다」라고
   적어뒀었다. 같은 사람을 두 군데서 다르게 부르면 센 쪽이 이긴다 */
eq('민현을 무례한 사람으로 안 적는다', /존댓말인데 무례하다/.test(workerSrc), false);
eq('민현의 결은 두 군데가 같은 말을 한다',
  (workerSrc.match(/선을 조금 넘는다/g) || []).length, 2);
/* 인물 프롬프트는 「모든 말을 한 단어로 자르지는 않는다. 필요한 말은 한
   호흡으로 이어서 한다」인데 말버릇에는 「열 자 안팎, 말끝을 자른다」라고
   적어뒀었다. 말버릇이 뒤에 오고 「반드시 지킬 것」이라 그쪽이 이긴다 */
eq('재언의 문장 길이를 두 군데서 다르게 안 적는다',
  /열 자 안팎|말끝을 자른다/.test(workerSrc), false);
eq('두 군데가 같은 말을 한다 — 한 호흡으로 잇는다',
  /필요한 말은 한 호흡으로 이어서 한다/.test(workerSrc)
  && /할 말은 한 호흡으로 잇고/.test(workerSrc), true);
/* 인물의 대화 예시에는 물음표가 있는데 말버릇에서는 안 쓴다고 했었다 */
eq('없는 규칙을 지어내지 않는다', /물음표를 잘 안 쓴다/.test(workerSrc), false);
