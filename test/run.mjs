...
/* ── 명시한 화자는 몰래 안 고친다 ──
   ...
   **영영 발동하지 않는다** — 검사에 닿기 전에 이미 고쳐졌기 때문이다.
   강현 방에서 모델이 sender:"jaeeon"을 내도 조용히 강현 말이 됐다.
...
eq('유저 이름표가 붙은 줄은 캐릭터가 말하지 않는다',
  texts('[R] 좋아 좋아\n[이강현] 뭐가 좋은데요', 'minhyun', BOTH), ['minhyun|뭐가 좋은데요']);
   ...
   메신저 DOM/CSS를 유지한다. 74b195c부터 붙은 ref 합성 UI도 복구 대상이다. */
{
  ...
  eq('즉시 빈값 저장은 등록 화면에만 붙는다',
    (ui.match(/saveEmptyNow/g)||[]).length,3);
...
/* ── ④ 강현의 옛 일기 — 병원 옥상 ──
   오프닝에서 강현을 만난 판에서만, 「저 알죠」 세 줄이 앉은 뒤 유저가 처음
   ...
   사람한테 점점 가까워지는 곡선이다. */
{
  ...
  /* 한 짝에 한 번. 두 번째로 뜨면 연출이 아니라 답장이 된다 */
  {
    ...
    /* 표에 없는 짝(재언·옥상 / 강현·보건실)은 화면이 그냥 안 뜬다 */
    eq('표에 없는 짝은 안 뜬다',
...
eq('웹 등록 화면은 엔터로 다음 칸에 넘어간다',
  ...
  && /onNext=\{\(\)=>\{const n=ENR_FIELDS\.findIndex/.test(web), true);
/* 수정창은 타이핑 중 태그를 갈아끼우지 않는다. 모바일에서는 그 순간 입력칸이
   등록 화면 좌표로 돌아가 길게 튀었으므로, 처음부터 끝까지 input 하나다. */
eq('프로필 수정창의 칸은 입력 중 같은 input을 유지한다', (() => {
  const s = web.slice(web.indexOf('function ProfileDialog'), web.indexOf('/* ── 방 목록'));
  return /<input className="blankin sunken"/.test(s) && !/<Blank value=/.test(s);
})(), true);
...
})(), true);
{
  ...
  eq('you.txt의 값과 조사는 작은 글자로 돌아가지 않는다',
    /\.youprofile \.ename,\.youprofile \.eline>\.blank,\.youprofile \.eline>\.blankin\{[^}]*font-size:13px !important/.test(css)
    && /\.youprofile \.etail\{[^}]*font-size:12px/.test(css), true);
  eq('you.txt 칸은 겹치지 않게 54px씩 떨어진다',
    /\.youprofile \.e-subject>\.blank,\.youprofile \.e-subject>\.blankin\{top:84px !important/.test(css)
    && /\.youprofile \.e-age>\.blank\{top:138px !important/.test(css)
    && /\.youprofile \.e-likes>\.blank,\.youprofile \.e-likes>\.blankin\{top:192px !important/.test(css), true);
  eq('you.txt 점선은 조사 뒤에서 이어진다',
    /\.youprofile \.etail\{[^}]*z-index:1[^}]*background:transparent/.test(css)
    && /\.youprofile \.e-subject::after\{left:68%;top:105px\}/.test(css), true);
}
eq('이름 변경은 변신 버튼으로 보인다',
  /변신! <span className="kao">⸜\( &gt;᎑&lt; \)⸝♡<\/span>/.test(web), true);
eq('변신 카모지는 좌우가 같은 일반 부등호 글꼴을 쓴다',
  /\.youprofile \.yourename \.kao\{[^}]*font-family:Arial,"Noto Sans Cherokee","Segoe UI Symbol",sans-serif/.test(web), true);
eq('변신 버튼이 이름과 세 선호를 한꺼번에 적용한다', (() => {
  const s = web.slice(web.indexOf('function ProfileDialog'), web.indexOf('/* ── 방 목록'));
  return /const transform=\(\)=>/.test(s) && /onRename\(t\)/.test(s)
    && /\["subject","likes","dislikes"\]\.forEach/.test(s)
    && /onSaveField\(k,v\)/.test(s) && /onClick=\{transform\}/.test(s);
})(), true);
eq('시작한 뒤에도 프로필을 변신할 수 있다',
  /const rename=n=>\{localStorage\.setItem/.test(web)
  && !/profile=\{profile\} onSaveField=\{\(k,v\)=>\{if\(loadWorld\(\)\)return/.test(web), true);
eq('변신 버튼은 카모지까지 완전한 흰색이다',
  /\.youprofile \.yourename,\.youprofile \.yourename \.kao\{\s*color:#fff !important;-webkit-text-fill-color:#fff !important\}/.test(web), true);
...
/* ── 둘이 같이 자는 시간이 없다 ──
   재언 1~6시, 강현 3~8시였다. 세 시부터 여섯 시까지 셋이 겹쳐서, 그 세 시간은
   유저가 말 걸 사람이 아무도 없었다. 강현이 자러 가는 시각과 재언이 일어나는
   ...
   「답하지 않는다」는 수가 없어서다. 같은 시계의 off를 반대쪽에서도 본다 */
{
  ...
  /* 단톡방은 한 사람만 깨 있어도 답이 온다 — 새벽 두 시의 강현이 그렇다.
     그리고 이제 둘이 같이 자는 시각이 없으므로(재언 1~4:30, 강현 4:30~8)
  ...
  eq('강현의 꺼짐도 같이 본다', D(two, { jaeeon:'집', minhyun:'꺼짐' }), ['jaeeon']);
   ...
   있는 사람이 옆방에서 말을 하면 그 점이 거짓말이 된다 */
{
  ...
  /* ── 자는 사람을 셋으로 세지 않는다 ──
     ...
     새벽 두 시엔 재언만 자고 강현은 세 시까지 깨 있다 — 목록에 「안 자는 중」이
...
/* ── 단톡방은 나중에 생긴다 ──
   ...
   알게 되는 근거는 새로 만들지 않는다. 이미 강현에게 보내고 있는 신호가 그거다 */
{
  ...
  eq('초대는 시스템창이다',
    ...
    && /이강현이 방을 만들고 당신을 넣었어요/.test(web), true);
  eq('왜 불렀는지는 안 알려준다',
...
/* ── 가방은 준 사람을 같이 들고 간다 ──
   ...
   하나뿐이었고 그건 전부 유저→인물이라, 강현이 제가 준 젤리를 두고
   ...
   첫인사를 보낸다 — 그래서 첫 화면에서 이미 둘의 시간대가 갈린다. */
{
     ...
     있을 수 있고, 학교 넷은 통째로 닫힌다. 그래서 뽑는다 */
  {
    ...
    /* 자리 임자와 만나는 사람이 어긋나면, 재언 자리에서 강현을 기다리게 된다 */
...
/* 강현은 네 시 반까지 깨 있다. 두 시로 잡아놨더니 새벽에 그가 먼저 말을 거는데
  ...
  /if\(mm>=22\*60\|\|mm<270\) return \{s:"on",  t:"안 자는 중"\}/.test(web), true);
{
  ...
  eq('주말 밤에도 강현은 깨 있다', P('minhyun', new Date(2026, 0, 10, 23)).t, '안 자는 중');
}
...
eq('주말은 안 보낸다 — 요일이 이미 실려 있다', /pr\.t!=="주말"/.test(web), true);
{
  ...
  eq('상태가 [지금] 줄에 실린다', v({ minhyun: '수업 중' }).includes('낮 · 이강현: 수업 중'), true);
  ...
  eq('단톡방은 둘 다 싣는다', buildVolatile('chat', 'group', 'R', null, [], null, { group: 5 },
    ...
    .includes('이재언: 보건실 · 이강현: 수업 중'), true);
...
/* 강현에게는 그 갈래가 없다. 저녁이면 그는 선톡이 아니라 자리에서 만난다 */
eq('강현은 시각을 안 본다', demo.demoGreetWhen(-1, 'minhyun', new Date(2026, 0, 6, 19)), '첫 만남');
...
/* 셀카. 강현만 보내고, 가까워지기 전에는 아낀다 — 처음부터 주면 그건
...
eq('강현이 밤에 먼저 건다', demo.demoProactive('minhyun', '밤', '윤하').length > 0, true);
   ...
   하는데 교생 실습이 월요일부터라 주말에는 그 자리가 없다. */
{
  ...
  /* 두 사람의 창이 다르다 — 재언은 퇴근까지, 강현은 야자까지.
     단 야자가 붙는 날이라야 그렇다. 1월 15일이 야자 주의 목요일이다 */
  eq('야자 날에는 재언이 퇴근해도 강현은 학교다',
    [D.roomLock(empty, 'jaeeon', at(15, 17)), D.roomLock(empty, 'minhyun', at(15, 17))],
    [D.WAIT_LINES, null]);
  /* 야자는 격주 목요일에만 붙는다. 없는 날 저녁에는 강현도 학교에 없다 —
     전에는 생활 리듬이 평일 저녁을 통째로 「야자」라고 불러서, 야자도 없는
     화요일 저녁에 안 만난 강현의 방이 열리고 선톡이 나갔다 */
  eq('야자 없는 평일 저녁에는 강현도 내일 만난다',
...
/* 사진 방아쇠는 재언 쪽만 센다. 강현이 셀카는 "저 지금 여기 왔어요"라서
   장소도 사건도 아니고, 관전방은 강현이 여는 방이라 물을 사람이 없다. */
...
/* 사진과 남은 날, 두 사건이 무엇을 못박는지. 강현이는 선생님 갤러리를 볼 수
...
eq('관전방은 강현이 연다', /첫 발화는 이강현이 한다/.test(workerSrc), true);
...
/* ── 인물은 인물 프롬프트에만 있다 ──
   ...
   오는 코드 쪽이 이긴다. 실제로 세 번 그랬다: 강현이 「무례하다」로,
...
eq('관계가 쌓이는 것이라고 못 박는다', (() => {
  ...
  return /유저와 보내는 하루가 쌓일 때마다 감정이 달라진다\. 이미 가까운 사이가 아니다\. 가까워질 수 있는 사이다\./.test(w)
    && w.indexOf('가까워질 수 있는 사이다') < w.indexOf('이재언과 이강현\n\n이재언은 이강현을') + w.length;
})(), true);
...
eq('바탕 그림이 그라데이션에 안 덮인다',
  /(?:^|\n)body\{[^}]*background(?:-image)?:linear-gradient/.test(web), false);
eq('바탕 그림이 저장소에 있다',
...
/* 한자만 흘리는 게 아니다. "Table of contents"가 강현의 말로 화면에 떨어졌다.
...
/* ── 메아리 ──
   유저: 「흥」 → 강현: 「흥이래요.」 유저가 방금 쓴 말을 그대로 옮기고 인용
   ...
   indexOf 필터가 세 갈래를 다 삼킨다 — 한 번 그랬다 */
{
    ...
    '새로 오셨죠. / 애들 때문에 정신 없으시겠네요. / 저한테는 편하게 메세지 주셔도 됩니다.');
  {
    ...
    eq('강현은 확인과 청구서로 문을 연다', open.join(' / '),
     ...
     묻고 나서 들으면 해명이고, 안 물었는데 들으면 고발이다. 순서가 전부다. */
  {
    ...
    /* 재언에게는 이 갈래가 없다 — 그 옥상은 강현만 아는 장면이다 */
...
/* ── 첫 만남 리텐콘 — 병원 옥상 ──
   ...
   않았고, 하늘을 보고 있다가 강현의 담배를 밟아 끄게 했다. 재활병원이라
   강현의 상담 기록과 같은 건물이다. 옛 설정이 한 조각이라도 남으면
   세계가 두 개의 첫 만남을 갖는다. */
{
  const wk = readFileSync(join(ROOT, 'worker.js'), 'utf8');
  eq('첫 만남은 병원 옥상이다',
    /이강현은 개학 전 재활 치료 중인 병원 옥상에서 유저를 만났다/.test(wk)
...
/* 「얼굴 보니까 방금 삼촌이랑 얘기하고 온 사람 얼굴이라」가 첫날에 나갔다.
   강현은 유저가 삼촌과 아는 사이인지도 모르는 데서 시작한다 — 눈치를
...
eq('처음부터 삼각형을 다 아는 건 아니라고 적어뒀다',
  /이강현은 처음부터 이재언과 유저의 과거를 알지 못한다/.test(workerSrc)
...
/* ── 지문을 없앴다 ──
   ...
   「강현은 그냥 눈을 감는다.」가 말풍선으로 나갔고, 유저가 「왜 자꾸
   ...
   역할 바로 다음에 대화 원칙과 쓰는 법이 오고, 세계·사연은 그 뒤로 물렸다. */
{
  ...
  /* ── 말꼬리와 우기기 ──
     재언은 꼰대로, 강현은 없던 일을 있었다고 말하는 쪽으로 기울었다. 둘 다
  ...
  /* ── 두 사람의 낯빛은 갈려야 한다 ──
     ...
     그런데 강현 설정에 이미 「낯빛이 흐리다 + 다크서클」이 있다. 둘 다
     희멀게지면 두 사람이 같은 인상이 된다. 재언은 관리된 창백, 강현은
     방치된 흐림 — 갈라놓은 것이 붙어버리지 않았는지 본다. */
  eq('두 사람의 낯빛이 안 겹친다', (() => {
    const 재언 = buildSystem('chat', 'jaeeon', 'R', null, [], null, null, null)[1].text;
    const 강현 = buildSystem('chat', 'minhyun', 'R', null, [], null, null, null)[1].text;
    return /낯빛이 희고 핏기가 없다/.test(재언) && /피부는 깨끗하다/.test(재언)
      && !/다크서클/.test(재언)
      && /낯빛이 흐리다/.test(강현) && !/핏기가 없다/.test(강현);
  })(), true);
  /* ── 안경은 재언만 쓴다 ──
     다시 뽑은 그림이 전부 얇은 은테를 쓰고 있어서 설정에 박았다. 그림과
     설정이 갈리면 인물이 사진마다 다른 사람이 된다. 강현 쪽에 번지지
     않았는지도 같이 본다 — 그쪽 얼굴 표지는 한쪽 이어폰이다. */
  eq('안경은 재언 설정에만 있다', (() => {
    const 재언 = buildSystem('chat', 'jaeeon', 'R', null, [], null, null, null)[1].text;
    const 강현 = buildSystem('chat', 'minhyun', 'R', null, [], null, null, null)[1].text;
    return /얇은 은테 안경을 쓴다/.test(재언) && !/안경/.test(강현)
      && /한쪽 이어폰을 자주 낀다/.test(강현);
  })(), true);
...
}
{
  ...
  eq('그 말이 가변부의 마지막 줄이다',
    v.trimEnd().endsWith('했던 요구를 되풀이하거나 대화를 착하게 닫지 않는다.'), true);
   ...
   워커는 진작부터 감당하고 있었다. 프론트 가드 한 줄이 막고 있었을 뿐이다. */
{
  ...
  /* ── 방은 갈래마다 다르다 ──
     ...
     안 받아서 관전이 **강현 1:1 방으로 처리**되고 있었다 — 클라이언트가
...
/* ── 관전 대화는 한 덩이로 서야 한다 ──
   ...
   「매점 가라며.」라고 강현이 하지도 않은 말을 인용했다. 그 앞은 영영 없다. */
{
  ...
  /* ── 인용 어미 ──
     「매점 가라며」가 나왔는데 강현은 바로 「가요, 가」로 받았다. 의도는
...
/* ── 이 세계의 계절은 안 돈다 ──
   요일과 때만 보내고 계절을 안 보냈더니 팔월에 강현이 「눈이 그제보다 덜
...
/* ── 두 사람이 서로에 대해 같은 것을 알아야 한다 ──
   ...
   「같이 살아요」라고 했고, 점심은 강현이 「집에서 먹는다」 재언이 「급식실
   간다」고 갈렸다. */
{
  ...
  eq('혼자 살던 것은 과거로 적혀 있다',
    /강현이 오기 전 혼자 살던 몇 해 동안 익혔다/.test(재언블록)
...
/* ── 수위는 인물마다 다르다 ──
   ...
   다른 사람에게도 걸린다. 재언은 스물아홉·스물여덟 성인 둘이고, 강현은 고3에
   ...
   인물 블록에서 정한다 — 조각 2가 이미 인물별로 갈려 있어 캐시도 안 깨진다. */
{
  ...
  const 강현 = buildSystem('chat', 'minhyun', 'R', null, [], null, null, null).map(b => b.text).join('');
  ...
  /* 강현 — 고3이고 유저는 그 학교 교생이다. 세지는 것은 몸이 아니라 집착이다 */
  eq('강현에게 성적인 말을 안 시킨다', /성적인 말은 하지 않는다/.test(강현), true);
  eq('강현은 안 놓아주는 쪽으로 센다',
    /\*\*안 놓아주는 쪽\*\*이고, 그게 더 무섭다/.test(강현), true);
  eq('강현은 거리를 좁히지 붙잡지 않는다',
    /붙잡는 게 아니라 \*\*거리를 좁히는 것\*\*으로 한다/.test(강현), true);
  /* ── 안 만지고 아는 것 ──
     몸에 관한 것도 닿아서가 아니라 봐서 안다. 「샴푸 바꿨죠」라고 하고
     어떻게 아느냐고 물으면 대답하지 않는다 — 이 애 설계가 원래
     「무서운 건 몸이 아니라 알고 있다는 것」이다 */
  eq('강현은 안 만지고 안다',
    /몸에 관한 것도 \*\*닿아서가 아니라 봐서\*\* 안다/.test(강현), true);
  eq('그 결의 예시가 있다',
    /강현: 샴푸 바꿨죠\. \/ 어제까진 다른 거였는데\./.test(강현), true);
  /* ── 조르되 받아내지 않는다 ──
     유저의 집은 INVITES에도 없다(편의점·레코드샵·체육관뿐). 이 애도 그걸
     알아서 조른다 — 받아낼 생각이 아니라 거절당하려고 하는 말이다.
     실제로 가는 일은 없다는 것을 같이 적어둔다 */
  eq('조르는 건 거절당하려고 하는 말이다',
    /받아낼 생각으로 하는 말이 아니라 거절당하려고 하는 말\*\*이다/.test(강현)
    && /실제로 가는 일은 없다/.test(강현), true);
  /* 한 줄로 이어져야 한다. 「…싶은데」에서 자르고 다음 변주로 넘기면
     말줄임표가 하는 일이 없어진다 — 조르기 전의 그 뜸이 이 말의 전부다 */
  eq('그 말의 예시가 있다',
    /강현: 선생님 머리카락에서 샴푸향 나요\. 저도 그거 쓰고 싶은데\.\.\. 초대해주세요\./.test(강현), true);
  eq('강현이 갈 수 있는 자리에 집이 없다',
    /minhyun: \[\{ at: 40, place: "편의점" \}, \{ at: 80, place: "레코드샵" \}, \{ at: 120, place: "체육관" \}\]/.test(workerSrc)
    && !/minhyun:[^\]]*"집"/.test(workerSrc), true);
  /* 재언 것이 강현에게 새면 안 된다. 조각이 갈려 있어야 그게 지켜진다 */
  eq('재언의 범위가 강현에게 안 샌다',
    ['키스를 장면 전환으로 끊지 않는다', '아침에 아직 같은 집'].filter(t => 강현.includes(t)), []);
}

/* ── 결이 흘러내리는 것 ──
     대화 예시는 고정부 한참 앞에 있고 이력은 뒤로 갈수록 길어진다. 그러면
     모델이 예시가 아니라 제가 몇 턴 전에 뱉은 밋밋한 말을 견본으로 삼는다.
     재언은 「자요」를 열 번 되풀이하는 잔소리꾼이 되고, 강현은 매 응답을
     「농담이고, 이 닦고 자요」로 닫는 착한 챗봇이 된다 — 둘 다 예시에는
     없는 말이다. 응답 직전인 여기서 한 번 더 붙잡는다 */
  eq('지난 말을 견본으로 삼지 말라고 한다',
    v.includes('「대화 예시」를 견본으로 삼는다'), true);
   ...
   없는 말투로 연기한다 — 데모(각본)와 라이브(모델)가 다른 사람이 된다. */
{
  ...
  /* 강현은 유저에게도 삼촌에게도 존댓말이다 */
  eq('문구집의 강현은 반말을 안 쓴다', M.filter(t => /(^뭐야|삼촌도 참|설명인데\.)/.test(t)).length, 0);
  eq('강현 말투 예시에도 반말이 없다', (() => {
...
eq('모든 인물 장소사진은 머리 위를 보존한다',
  ...
  && /\.scenewrap\.glasswindow\.scene-person-shot\{background-position:center top !important\}/.test(web), true);
...
/* ── 이름표가 말풍선 안으로 새는 것 ──
   ...
   「[이재언] 말」로 넣어주다 보니 모델이 그 모양을 따라 text에 「강현: 」을
   박아 보낸다. 말풍선마다 이름이 찍히고 sender는 다 같아서 아바타가 뭉친다. */
{
  ...
  eq('쌍점 이름표를 뗀다', one('minhyun', '강현: 네네.').text, '네네.');
  ...
  eq('이 방에 없는 사람은 말하지 못한다',
    [one('jaeeon', '강현: 삼촌 뭐해요', ['jaeeon']).sender, one('jaeeon', '강현: 삼촌 뭐해요', ['jaeeon']).text],
    ['jaeeon', '삼촌 뭐해요']);
  eq('이름표뿐인 줄은 버린다', unlabel([{ sender: 'minhyun', text: '강현:' }], B).length, 0);
  ...
  [placeGiver('집', false, 'jaeeon'), placeGiver('집', false, 'minhyun')], ['key', null]);
{
  ...
  /* 재언은 보건실에 있는 게 일이고 강현은 교실에 앉아 있다. 「불러줘서 왔어요」가
  ...
  /* 임자가 있는 자리 셋 — 교실은 강현, 보건실과 집은 재언이다.
     집은 재언 집이지만 강현도 산다. 사는 것과 임자인 것은 다르다 —
     여벌 열쇠를 내주는 쪽이 임자다. */
...
eq('단추 셋이 한 규칙을 쓴다',
  ...
  && (web.match(/className="(backbtn|giftbtn|sendbtn) rbtn"/g) || []).length === 5, true);
...
eq('채팅방 X가 목록으로 보낸다',
  /<WindowChrome title=\{room\.name\+\(watch\?"\.cam":"\.chat"\)\} onClose=\{onBack\}\/>/.test(web), true);
/* 안 눌려도 되는 것은 오프닝의 가짜 오류창 넷과 앱 창틀뿐이다.
   세계 확정 창의 X는 되돌아가기다 — 이 앱에 back 위젯은 어디에도 없으므로
   창틀이 그 일을 한다. 등록 화면도 X가 생기면서 여섯에서 다섯이 됐다 */
eq('공용 창틀도 X 부품을 그대로 쓴다',
  /function WindowChrome\([^)]*onClose[^)]*\)[\s\S]{0,180}<WinDots onClose=\{onClose\}\/>/.test(web), true);
/* 제목은 띠 안의 글자가 아니라 띠 위에 얹은 한 줄이 됐다 —
   띠가 유리라 안쪽에 빛막이 흐르고, 글자를 그 안에 두면 막에 먹힌다 */
eq('등록 화면 X도 눌린다',
  /<ProfileFrame title="NULL\.exe" onClose=\{onClose\}>/.test(web)
  && /function ProfileFrame\([^)]*onClose[^)]*\)/.test(web), true);
...
eq('X는 접기다', /<WindowChrome title=\{scene\.place\} onClose=\{onMinimize\}\/>/.test(web), true);
...
/* ── 사고가 대사로 새어 나갔다 ──
   화면에 이강현의 말풍선으로 이 두 줄이 떴다: 「…어긋남을 짚어야 함.
   ...
   들여다보게 되는 일이다. 인물이 절대 안 하는 셋으로 가려낸다. */
{
  ...
  eq('장치 이름과 유저라는 말을 버린다',
    ['유저가 뭐라고 했는데요', '눈치 신호를 보면', '사진키를 쓴다', '직전 문맥에 답한다']
      .filter(keep), []);
  /* 영문으로도 샜다 — 「user제일 좋은 자리로 예매해」가 강현 말풍선으로 떴다 */
  eq('영문 user도 버린다',
  ...
  /* 상대 이름은 그냥 쓴다 — 강현이 삼촌 얘기를 하는 건 대사다 */
  eq('상대 이름은 안 버린다', [
    ['minhyun', '이재언 삼촌이요?'],
  ...
  /* ── 성을 뗀 이름 ──
     ...
     아니라 「강현은」이라고 썼고, 유저가 「너 왜 자꾸 해설해」라고 묻자
     「해설 아니에요」 하고 세 번 더 했다(NULL_____4.txt 끝의 키스 장면). */
  eq('성을 떼도 제 이름이면 버린다', [
    ['minhyun', '강현은 잠깐 아무 말도 하지 않는다.'],
    ['minhyun', '강현은 그냥 눈을 감는다.'],
    ['minhyun', '강현은 그냥 웃는다. 짧게, 소리도 별로 안 낸다.'],
    ['minhyun', '강현이 잠깐 말을 멈춘다.'],
    ['jaeeon',  '재언은 여기서 더 말하지 않는다.'],
  ].filter(([w, t]) => keepAs(w, t)), []);
  /* 성을 떼도 상대 이름은 그냥 쓴다 — 강현이 삼촌 얘기를 하는 건 대사다 */
  eq('성을 떼도 상대 이름은 안 버린다', [
    ['minhyun', '재언이 삼촌이요?'],
    ['minhyun', '재언 삼촌은 그런 말 안 해요.'],
  ].filter(([w, t]) => !keepAs(w, t)), []);

  /* ── 이름이 안 든 지문 ──
     주어를 보는 표는 이름이 없으면 못 잡는다. 같은 판에서 이렇게 나갔다.
     맺음이 잣대다 — 소설의 지문은 현재 서술형(-ㄴ다/-는다)으로 맺고
     이 둘은 말끝을 그렇게 안 맺는다. */
  eq('이름 없는 지문도 버린다', [
    ['minhyun', '말 뒤에 잠깐 조용해진다. 동전 몇 개를 주머니에서 꺼내 만지작거리다 내민다.'],
    ['jaeeon',  '잠깐 아무 말도 하지 않는다.'],
    ['minhyun', '그냥 눈을 감는다.'],
  ].filter(([w, t]) => keepAs(w, t)), []);
  /* 넓지 않다는 것은 재봤다 — 문구집·데모 8242줄 중 걸리는 것이 없다.
     받침이 ㄴ이 아니면 안 걸린다 */
  eq('-ㄴ다가 아닌 맺음은 남는다', [
    ['minhyun', '그만 웃겨야겠다.'],
    ['jaeeon',  '감사합니다.'],
...
eq('쉬는시간은 학교 안이지만 교실 CLASS 잠금은 푼다', (() => {
  const src = web.slice(web.indexOf('const weekNo='));
  const P = new Function(src.slice(0, src.indexOf('\n}\n') + 3) + '\nreturn presence;')();
  return P('minhyun', new Date(2026, 0, 6, 9, 35)).t === '쉬는시간'
    && /const AT_WORK=\["보건실","수업 중","쉬는시간","점심","야자"\]/.test(web);
})(), true);
...
/* ── 귀갓길 ──
   ...
   붙는 한 다리가 그 일을 한다. 재언은 태워다 주고 강현은 같이 버스를 탄다. */
{
  eq('귀갓길은 지도에 없다',
...
/* ── 내보낸 파일 ──
   「이강현이 이어폰을 받았다」는 아무도 한 말이 아닌데 유저 이름이 붙어서
...
/* 상태값은 작업용 영어 대신 화면의 한국어를 쓴다. 저장 키는 그대로다. */
   ...
   이미 있는 생활 리듬이 정한다 — 새 규칙을 만들지 않는다 */
{
  ...
  /* 새벽 두 시 — 재언은 자고 강현만 깨 있다 */
  eq('새벽엔 강현만 있다', F(new Date(2026, 0, 7, 2)), ['minhyun']);
...
/* ── 학교는 사람이 있을 때만 학교다 ──
   ...
   강현은 집에 갔는데 교실 문이 열려 있었다. 시각표를 요일마다 새로 적지 않고
   생활 리듬(atWorkNow)을 그대로 본다. 둘 중 좁은 쪽이 이긴다. */
{
  ...
  /* 1/7 수 — 야자가 없는 날. 강현은 16:20에, 재언은 17:00에 학교를 뜬다 */
  ...
  /* 1/15 목 — 야자가 붙는 날. 강현이 남으므로 그 자리들만 열린다.
     보건실은 재언 자리라 17시에 닫히고, 체육관은 hours(18)가 먼저 끊는다 */
  eq('야자 날 저녁에는 강현 자리가 열린다', row(at(15, 19)), ['학교', '교실', '옥상']);
   ...
   또 서 있으면 안 만난 사람의 방은 계속 잠긴 채로 남는다. */
{
  ...
  /* 1/15 목 19시 — 야자라 강현은 학교에 있고 재언만 밖이다 */
  eq('한 사람만 나올 수 있는 시간 — 오프닝이 강현이면 재언이 선다',
    out('minhyun', at(15, 19)), ['jaeeon']);
  eq('같은 시간 — 오프닝이 재언이면 아무도 안 선다',
    out('jaeeon', at(15, 19)), []);
  /* 새벽 두 시 — 재언은 자고 강현만 깨어 있다 */
  eq('새벽에는 강현만 — 오프닝이 재언이면 강현이 선다',
    out('jaeeon', at(7, 2)), ['minhyun']);
  eq('새벽에 오프닝이 강현이면 아무도 안 선다', out('minhyun', at(7, 2)), []);
   ...
   길보다 자연스럽고, 잠긴 자리도 자물쇠 딱지가 아니라 안 열리는 문이 된다. */
{
  ...
  eq('다녀온 문의 분홍 발광을 걷는다', /\.desk \.cabdoor\.been img\{filter:none !important\}/.test(web), true);
   ...
   의식이 된다. 그 뒤로는 peek 옆 단추가 지금이 몇 교시인지 들고 있다. */
{
  ...
  const yajaSrc = web.slice(web.indexOf('const weekNo='), web.indexOf('const PERIODS='));
  const periodsSrc = web.slice(web.indexOf('const PERIODS='), web.indexOf('\n\n/* ── 접속 상태 ──'));
  const src = web.slice(web.indexOf('const LEAVE_AT='), web.indexOf('/* 하루의 경계는'));
   ...
   보내는 것도 남이 찍어줘야 나오는 그림이다. 자기가 찍을 수 있는 것만 남긴다 */
{
  ...
  /* 재언은 사진을 한 장도 안 보낸다. 있는 그림이 전부 본인이 프레임 안에 있고
     그건 남이 찍어줘야 나온다. 이 사람 몫은 배경이 한다. 강현은 셀카를 찍는다 */
   ...
   정돈이 갑옷인 사람이라 말투가 흔들리는 것 자체가 설정과 어긋난다 */
{
  ...
  eq('재언은 조카에게 반말로 고정이다',
    /이강현에게는 반말을 쓴다\. 한 대화 안에서 존댓말과 섞지 않는다/.test(wk), true);
   ...
   것이다. 규칙을 더 쓰는 대신 견본을 고쳤다(작가가 쓴 줄이다) */
{
  ...
  eq('강현 견본은 부정으로 열지 않는다',
    /강현: 알았어요\. 안 그럴게요\. \/ 그럼 어디까지 괜찮아요\?/.test(ex)
    && /강현: 그럼 그냥 있을게요\. \/ 그냥 있는 건 괜찮잖아요\./.test(ex), true);
  eq('되받아치던 두 줄은 없앴다',
...
/* ── 방금 받은 것이 빠진 가방 ──
   체육관에서 손목 보호대를 받고 나온 턴에서 강현이 「손목 보호대가 왜
   ...
   그게 문제라고 알려줬다. 자는 생산 코드와 다른 눈이어야 한다. */
{
  ...
  /* ── 부정은 방향을 지킨다 ──
     둘을 한 자루에 넣었더니 「강현이한테 잘해준 적 없다니까요」가
  ...
  eq('다른 동사의 활용은 안 잡는다',
    seesDenial('강현이한테 잘해준 적 없다니까요.', '', 'from_char'), null);
  ...
  eq('작가 문구집에서 하나도 안 걸린다', (() => {
    ...
    for (const raw of md.split('\n')) {
      const m = raw.match(/^　(재언|강현|둘|해설)\s*—\s*(.+?)\\?$/);
      if (!m) continue;
      const t = m[2];
      if (seesLeak(t) || seesTail(t) || seesUserWrite(t)) bad.push(t.slice(0, 40));
    }
   ...
   프롬프트**를 들여다본다. */
{
  ...
  /* ── 1. 재언만 아는 선물 출처가 강현에게 안 보인다 ── */
  await (async () => {
    const r = await runTurn({ mode: 'chat', room: 'minhyun', ...BASE, ...GAVE_JAEEON });
    eq('요청이 실제로 돈다', r.status, 200);
    const saw = writerSaw();
    eq('강현은 출처를 못 본다', leaksOrigin(), false);
    eq('강현은 물건이 있다는 것만 본다', saw.includes('이재언에게 회색 머그컵이 있다'), true);
  })();
  ...
  await (async () => {
    ...
    eq('재언도 물건만 본다', saw.includes('이강현에게 편지지가 있다'), true);
  })();
  ...
  /* ── 물건의 정체성은 (수신자, 종류)다 ──
     ...
     같은 머그컵을 재언에게 하나, 강현에게 하나 줄 수 있다. 코드도 그렇게
  ...
  /* ── 같은 종류를 둘에게 각각 하나씩 — 정상이다 ── */
  {
    ...
    eq('각자 하나씩 가진 것으로 읽힌다',
      ...
      ['선생님이 이재언에게 회색 머그컵을 줬다.',
       '이재언에게 회색 머그컵이 있다.', '이강현에게 회색 머그컵이 있다.']);
  }
  ...
  await (async () => {
    ...
    eq('ack 뒤에도 누구인지가 남는다 — 반대 방향',
      writerSaw().includes('이재언은 유저가 이강현을 상대로 정했다는 것을 안다'), true);
  ...
  await (async () => {
    const ask = (room, partner, extra) => runScene({ mode: room === 'health' ? 'auto' : 'chat',
      ...
      room === 'health' ? '[이재언] 네.' : room === 'group' ? '[이강현] 네.' : '네.');
    ...
    /* 모르는 방 이름은 대화는 강현 방으로 살리되, 장면은 그 위에서 안 태운다 */
   ...
   유저가 거절해도 들어가고, 인물이 준 적 없는 것이 가방에 있었다. */
{
     ...
     으로 흉내낸다 — 그게 실제로 일어나는 일이다. */
  {
    ...
    /* ── 시계도 가짜여야 한다 ──
       ...
       본다. 강현이 자는 시각에 돌리면 요청이 모델까지 안 가고 「자고 있다」로
       ...
       건네는 대사가 뜨기도 전에 「받았다」가 먼저 떴다. */
    {
      ...
      eq('건네는 대사가 먼저, 받았다가 나중이다',
        said(W), ['뭐해요', '이거 드세요', '따뜻해요', '· 이강현에게 캔커피를 받았다']);
    ...
    /* ── 같은 답을 두 번 재생해도 한 번이다 ── */
    {
      ...
      await W2.tick(5000);
      eq('끝난 답은 다시 재생 안 된다', said(W2), ['뭐해요', 'ㄱ', '· 이강현에게 캔커피를 받았다']);
   ...
   움직여야 그 사고가 문법 오류가 된다. */
{
  ...
  /* ── D1 SENDER 복구 ──
     ...
     못했다. 강현 방에서 sender:"jaeeon"이 조용히 강현 말이 됐다. */
  ...
  /* 같은 종류를 재언과 강현이 각각 가진 정상 상태 — 남의 것을 부정한 것이
     내 것 부정으로 세면 안 된다. 늘 (수신자, 종류)를 함께 본다. */
  eq('남의 같은 물건과 안 섞인다',
    hardFilter(say('강현이 회색 머그컵은 저한테 없어요.', 'minhyun'), BOTH,
  ...
  eq('앞 방의 결과가 다음 방에 안 남는다', (() => {
    const a = parseMessages('[이재언] 앉으세요.', 'minhyun', MH);        // 강현 방 → 난입
  ...
  eq('두 경로가 같은 계약을 낸다', (() => {
    ...
    return keys(parseMessages('{"messages":["ㄱ"]}', 'minhyun', MH))
        === keys(parseMessages('[이강현] ㄱ', 'minhyun', MH));
  })(), true);
   ...
   첫 마디가 그날 영영 안 나온다. 자리가 그 사람의 첫 마디를 삼킨 것이다. */
{
  ...
  /* 자리에서 만나도 그 사람의 결은 같아야 한다. 재언은 용건부터 만들고,
     강현은 아는 걸 아는 채로 연다. 옥상은 여기서 설명하지 않는다 —
...
/* ══════════ 이야기 상태와 장면 감지 — E-B ══════════
   ── 왜 상태 기계인가 ──
   「어디서 만났지」를 물었는데 강현이 「그럼 그냥 모르는 사람이네요」로
   ...
   아무도 모른다. 클라이언트는 화면의 선택(D-0·WHO)만 예약한다. */
{
  ...
  /* ── 첫 선톡 바로 뒤의 정식 답들 (문구집 4019의 게이트) ──
     강현의 「저 알죠? / 책임진다면서요」 뒤에 유저가 실제로 치는 답은
     ...
     누구인지가 없으면, 재언은 자기가 선택됐는지도 모른다. */
  {
    ...
    eq('반대 방향도 정확하다', rev.includes('유저가 이강현을 상대로 정했다'), true);
  ...
  /* ── 지킴은 「약속의 동사가 완료형으로 답에 있다」, 거둠은 그 말을 접는 말 ──
     두 판이 적대 검증에 깨졌다. 답의 모양만 보던 판은 「까먹었어요」를 아무
     약속에나 거둠으로 읽었고, 낱말 겹침 + 아무 완료형을 보던 판은 「들어볼게요」
     뒤의 인사 「잘 들어갔어요?」와 「챙겨줄게요」 뒤의 「아침 챙겨 먹었어요」를
     지킴으로 읽었다(실제 기록 43,650쌍에서 true 47, 지킴 0). 지금은 약속의
     동사를 완료형으로 바꿔(갈게요→갔·왔, 들어볼게요→들어봤·들었) 그 꼴을 찾는다 */
  const GO = '늦으면 데리러 갈게요';
  eq('약속의 동사가 완료형이 된다', [
    ENG.promiseKeptForms(GO).includes('데리러\\s*왔'),
    ENG.promiseKeptForms('CD 한번 들어볼게요').includes('들었'),
    ENG.promiseKeptForms('보리차 끓여 놓을게요').includes('끓여\\s*놓은'),
    ENG.promiseKeptForms('내일 씹을게요'), ENG.promiseKeptForms('내일 전화할게요').includes('전화했'),
    /* 한 글자 완료형은 앞말이 붙어야 한다 — 「다시」는 앞말이 못 된다 */
    ENG.promiseKeptForms('다시 올게요').some(f => /^왔$|\\s\\*왔$/.test(f)),
  ], [true, true, true, ['씹었'], true, false]);
  eq('실제 기록의 지킴·거둠은 그 약속에 걸린다', [
    ['CD 한번 들어볼게요', 'CD, 방금 겨우 한 번 들었어요.'],
    ['야자 끝나면 한 번 들어볼게요', '들었어요, 아까.'],
    ['보리차 끓여 놓을게요', '보리차 끓여놓은 거 있어요.'],
    ['보리차 더 끓여둘게요', '보리차 끓여놓은 거 있어요.'],
    [GO, '데리러 왔어요.'], [GO, '데리러 왔어요. 아직 안 나왔어요?'],
    [GO, '데리러 왔으니까 나와요.'], [GO + ', 걱정 마요', '데리러 왔어요.'],
    [GO, '약속 깼어요. 미안해요.'],
    ...
    ['내일 전화할게요', '전화했어요, 아침에.'],
    ['내일 씹을게요', '껌 씹었어요, 아까.'], ['이따 먹을게요', '먹었어요.'],
    ['내일 구워 줄게요', '구웠어요.'], ['내일 챙겨줄게요', '챙겼어요.'],
    ['내일 LP 들려줄게요', '들려줬어요.'], ['그러면 탕수는 한번 먹어볼게요', '탕수 먹었어요.'],
    [GO, '그 말은 없던 걸로 해요.'], [GO, '약속 못 지키겠어요.'],
    [GO, '못 가겠어요, 미안해요.'], ['다시 올게요', '그 말은 없던 걸로 해요.'],
  ].filter(([p, t]) => !ENG.promiseTouched(p, t)), []);
  /* 적대 검증이 실제 기록에서 꺼낸 오탐들 — 앞 판에서 전부 닫혔다 */
  eq('딴 동사·물음·꺼내기만 한 말·미룬 말·일반 답은 안 걸린다', [
    ['야자 끝나면 한 번 들어볼게요', '잘 들어갔어요?'],
    ['야자 끝나면 한 번 들어볼게요', '새벽 두세 시쯤 들어왔어요, 그때는.'],
    ['야자 끝나면 한 번 들어볼게요', '저 오늘 야자 중에 배터리 나갔거든요.'],
    ['비 오면 더 챙겨줄게요, 그냥', '아침 챙겨 먹었어요.'],
    ['비 오면 더 챙겨줄게요, 그냥', '밥은 챙겨 먹었어요?'],
    ...
    [GO, '데리러 갔다 올게요.'], [GO, '데리러 갔던 날 있잖아요.'],
    [GO, '데리러 갈게요, 내일은 꼭.'], ['내일 챙겨 올게요', '안 챙겨 왔어요.'],
    ['내일 챙겨 올게요', '챙겨 온다는 게 깜빡했어요.'], [GO, '데리러 가지 않았어요.'],
    [GO + ', 걱정 마요', '걱정했어요, 많이.'], [GO, '약속 깨면 안 되죠.'],
    ['보리차 끓여 놓을게요', '보리차는 아직이에요.'],
    ['CD 한번 들어볼게요', 'CD는 아직 못 들어봤어요, 선생님.'],
    ['CD 한번 들어볼게요', 'CD는 계속 주머니에 있어요.'],
    ['다시 올게요', '왔어요.'], ['이따 먹을게요', '밥은 먹었어요?'],
...
/* ── ③ 재언 일기의 빈칸은 서버로 안 간다 ──
   문서의 「서버 전달 경계」다. 서버로 가는 빈칸은 강현 온보딩(④) 하나뿐이고,
...
/* 확정 문안 — 글자 그대로다. 한 글자라도 바뀌면 20년 전 그 아이의 글이 아니다 */
  ...
  eq('사탕을 누구에게 줬는지는 안 적는다',
    D.DIARY_LINES.some(l => /목걸이|재언|강현|삼촌/.test(l)), false);
}

/* ══════════ ④ 엽서 뒷면 — 나가는 유일한 빈칸 ══════════
   문서의 「서버 전달 경계」: 서버로 가는 빈칸은 강현 온보딩(④) 하나뿐이고
   ...
   좋겠는가」 한 자리뿐이고, 인물에게는 조립된 문장 한 줄이 간다. */
{
  ...
  eq('빈 자리는 그대로 둔다', D.hidMask('강현 상담 기록 · 1'), '□□ □□ □□ □ □');
   ...
   캐릭터 대사를 새로 하드코딩하지 않는다. */
{
  ...
  eq('강현의 첫 연락이 정확히 세 줄이다',
    corpus.includes('강현 — 선생님. / 저 알죠? / 선생님이 저 책임진다면서요.'), true);
