   ...
   데이터가 먼저, 화면 조각이 다음, 앱이 마지막. 하나라도 어긋나면 화면이 하얗다. */
{
     ...
     보이고 사람은 안 고쳐졌다고 한다. 모든 파일이 같은 번호여야 한다 */
  {
    ...
    eq('판 번호가 지금 내용의 것이다',
      [v[0][1], seal.digest('hex').slice(0, 12)], ['236', '9af6d5203c0f']);
  ...
  eq('데이터는 바벨을 안 탄다', WEB_DATA_FILES.every(f =>
    html.includes(`<script src="${f}?v=236"></script>`)), true);
  eq('화면과 앱은 바벨을 탄다',
    [...WEB_UI_FILES, 'scripts/game.js', 'app.js'].every(f =>
      html.includes(`<script type="text/babel" src="${f}?v=236"></script>`)), true);
