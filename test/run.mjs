   ...
   데이터가 먼저, 화면 조각이 다음, 앱이 마지막. 하나라도 어긋나면 화면이 하얗다. */
{
     ...
     보이고 사람은 안 고쳐졌다고 한다. 모든 파일이 같은 번호여야 한다 */
  {
    ...
    eq('판 번호가 지금 내용의 것이다',
      [v[0][1], seal.digest('hex').slice(0, 12)], ['241', 'dd0a9ba3a1be']);
  ...
  eq('데이터는 바벨을 안 탄다', WEB_DATA_FILES.every(f =>
    html.includes(`<script src="${f}?v=241"></script>`)), true);
  eq('화면과 앱은 바벨을 탄다',
    [...WEB_UI_FILES, 'scripts/game.js', 'app.js'].every(f =>
      html.includes(`<script type="text/babel" src="${f}?v=241"></script>`)), true);
   ...
   들여다보게 되는 일이다. 인물이 절대 안 하는 셋으로 가려낸다. */
{
  ...
  /* ── 성을 뗀 이름 ──
     표가 풀네임만 들고 있어서 한 판이 통째로 샜다. 모델은 「이민현은」이
     아니라 「민현은」이라고 썼고, 유저가 「너 왜 자꾸 해설해」라고 묻자
     「해설 아니에요」 하고 세 번 더 했다(NULL_____4.txt 끝의 키스 장면). */
  eq('성을 떼도 제 이름이면 버린다', [
    ['minhyun', '민현은 잠깐 아무 말도 하지 않는다.'],
    ['minhyun', '민현은 그냥 눈을 감는다.'],
    ['minhyun', '민현은 그냥 웃는다. 짧게, 소리도 별로 안 낸다.'],
    ['minhyun', '민현이 잠깐 말을 멈춘다.'],
    ['jaeeon',  '재언은 여기서 더 말하지 않는다.'],
  ].filter(([w, t]) => keepAs(w, t)), []);
  /* 성을 떼도 상대 이름은 그냥 쓴다 — 민현이 삼촌 얘기를 하는 건 대사다 */
