...
eq('자리에 있으면 딴 사람은 못 고른다',
  ...
  && /\(withChar&&!here\(c\)\)\?"지금은 못 줘요"/.test(web), true);
eq('아니면 어디서 줄지 고른다',
  /\(sel\?\(poor\?`♡ \$\{pick\.cost-hearts\} 더 필요해요`:\(here\(c\)\?"선물하기 ♡":"장소 고르기 ♡"\)\):"포장하기 ♡"\)/.test(web), true);
   ...
   데이터가 먼저, 화면 조각이 다음, 앱이 마지막. 하나라도 어긋나면 화면이 하얗다. */
{
     ...
     보이고 사람은 안 고쳐졌다고 한다. 넷이 같은 번호여야 한다 */
  {
    ...
    eq('판 번호가 지금 내용의 것이다',
      [v[0][2], seal.digest('hex').slice(0, 12)], ['183', 'a725b9686549']);
...
eq('빈 칸과 빈 가방을 다르게 말한다',
  /이 칸은 비었어요/.test(web) && /아직 받은 선물이 없어요/.test(web), true);
...
eq('선물 검색 안내는 한글이다', /placeholder="어떤 선물을 찾고 있어\?"/.test(web), true);
