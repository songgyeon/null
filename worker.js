...
민현: 안 붙었는데요. 지나간 건데. / 그럼 어디까지가 가까운 건데요, 알려줘요.
...
유저: 기다리지 마 / 기다리지 말라니까
민현: 기다리는 거 아니에요. 그냥 안 가는 거지. / 그거 선생님이 정하는 거 아니잖아요.
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      /* ── 상한과 사고 ──
         ...
         2400도 작았다. high 사고는 프롬프트 2만 자를 보고 1,000~2,000 토큰을
         쉽게 쓴다. 그러면 답 몫이 또 몇백밖에 안 남는다 — 사고가 답을 밀어내는
         구조가 그대로다. 사고가 얼마를 쓰든 답이 굶지 않을 만큼 잡는다.
         상한을 크게 잡아서 드는 값은 0이다. 안 쓰면 청구가 없고, 실제로 쓰면
         그건 사고가 그만큼 필요했다는 뜻이다. 작게 잡아서 얻는 것은 절약이
         아니라 잘린 답이었다 — 사고 값은 다 내고 쓸 수 없는 답을 받았다.
         스트리밍을 안 쓰므로 무한정 키우지는 않는다. 이 정도가 응답 시간과
         HTTP 시한 안에서 안전한 자리다.
         그래도 잘리면 한 번 더 크게 뽑는다 — 잘린 답을 화면에 내보내는 것보다
         다시 뽑는 쪽이 싸다. 잘린 턴도 청구는 됐는데 쓸 수 없으니까. */
      let raw = await askClaude(env, system, msgs, mode === "auto" ? 16000 : 12000);
      if (lastUsage && lastUsage.stop_reason === "max_tokens") {
        console.log("[NULL] 상한에 잘렸다 — 더 큰 그릇으로 한 번 더");
        raw = await askClaude(env, system, msgs, mode === "auto" ? 24000 : 20000);
      }
      /* ── 관찰용. 원인을 잡으면 뺀다 ──
