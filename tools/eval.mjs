...
/* ── 기록 읽기 ──
   ...
   시각은 「밤에 학교 가라고 했나」를 재는 재료다.

   ── 시각 판정은 이 파일이 아니라 시계 자가 한다 ──
   F(시계 통일)가 끝나 내보내기 표기는 이제 게임 시계다. 표기와 저장 ts의
   어긋남은 tools/eval-scenes.mjs의 scoreClock이 gameAt(ts) 기준으로 잰다 —
...
console.log('장면 갈래 지표는 여기서 안 잰다 — 텍스트 내보내기에는 라우팅 결과가 안 실린다.');
console.log('  기억 공개 사건당 정사 부정 · 중요 장면당 올바른 라우팅 · 시각 어긋남은');
console.log('  tools/eval-scenes.mjs가 trace JSON과 시계 fixture로 잰다 (--trace / --clock).');
