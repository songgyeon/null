...
/* ── 선물 ──
   ...
   강현은 안 쓴다고 해놓고 쓰겠다고 한다. 그게 이 둘이 고마워하는 방식이다.
...
var DEMO_GIFT = {
  ...
  bandana: {
    jaeeon:  [['이건 저보다 강현이가 하겠는데요.', '일단 받아둘게요.']],
  ...
  ramen: {
    jaeeon:  [['이건 강현이가 좋아하겠네요.', '저는 따로 챙겨 먹을게요.']],
...
var DEMO_GIFT_ANY = {
  jaeeon:  [['받을 이유가 없는데.', '그래도 받을게요.']],
  minhyun: [['저 주는 거 맞아요?', '그럼 가질게요.']],
};
...
/* ── 「두 사람」 방의 첫 장면 ──
   ...
   조카가 「삼촌」이라 부르고, 삼촌은 반말로 약을 챙긴다. 그거면 된다.
   여기서는 유저 얘기가 안 나온다. 첫 장면이 할 일은 둘의 관계를 보여주는
   것 하나뿐이고, 유저 얘기가 섞이면 그게 흐려진다 — 나머지 관전 장면들이
   그 몫을 한다. */
var DEMO_WATCH_OPEN = '아침에 약 얘기가 나오는 날';
...
function demoGreetWhen(gapMin, room, now = null) {
  ...
  /* 저녁에 처음 켠 사람은 그날 첫 자리에서 강현을 만난다. 재언의 첫인사는
     하루가 끝난 뒤에 오므로 시제가 바뀐다 — 앞일을 짐작하는 말이 아니라
     지난 일을 묻는 말이 된다. 강현에게는 이 갈래가 없다(저녁 시작이면
     그는 선톡이 아니라 자리에서 만난다). */
  if (!(gapMin >= 0)) {
