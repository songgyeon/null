...
그래서 지시는 「젊게」가 아니라 **「강현과 같은 화풍으로」**다. 이게 제일 크고,
아래 얼굴 항목들은 그다음이다.
...
**하나 — 강현과 갈라야 한다.** 강현 설정에 이미 「이목구비보다 낯빛이
...
| 이강현 | **방치된 흐림.** 고르지 않은 낯빛, 다크서클. 자기 몸 챙기는 회로가 꺼져 있던 쪽이다 |

지금 그림은 이게 거꾸로다. 재언이 거칠고 강현이 말끔하다.
...
강현도 같은 구조다 — 프로필 1, 배경 5(`shop` → `lp` → `bus` → `cat` →
...
프로필 화면이 둘 다 이 파일이다. 여기부터 잡고, 결과가 강현 옆에 놓아도
어색하지 않으면 다음으로 간다.
