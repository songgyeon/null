...
### 쓰는 손만 되돌리기 — `ENGINE_MODE=sonnet45`

기본 배선에서 **쓰는 자리에 앉는 모델 하나만** 옛 상급 Writer로 되돌린다.
...
화면과 trace에는 `engineLabel`이 `sonnet45`라고 적는다 — 배선 이름만 적으면
대시보드가 「gpt41」이라고 거짓말을 한다. `/?k=…` 간이 점검 페이지에서 지금
어느 손이 떠 있는지 바로 보인다.

되돌리기는 대시보드에서 `ENGINE_MODE`를 지우면 끝이다. 배포가 필요 없다.

배선이 정말 같은지는 시험이 실행으로 잰다 — 같은 packet 일곱을 두 깃발로
돌려 단계 순서와 응답 코드를 통째로 비교한다(`test/default-engine.test.mjs`).
