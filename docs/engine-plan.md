...
### selected-v1 — 말투 견본과 이번 턴의 재료 (구현됨, 실험 전용)

모델 배치를 바꾸지 않는다. **hybrid-pair 그대로**에서 Writer가 밋밋한
안전문으로 도망가지 않도록 재료를 주는 갈래다(`DIALOGUE_RULESET=selected-v1`).
운영 기본값은 꺼짐이고, 호출 수·모델·판 번호는 그대로다.

세 가지가 붙는다.

**① 이번 턴 재료 하나.** 「설레게 써라」는 하급 모델에게 안 먹힌다. 형용사는
방향을 못 준다. 밋밋한 답은 **물 것이 없을 때** 나오므로, 코드가 이번 턴에
실제로 있는 것 하나를 골라 준다(`turnMaterial`). 우선순위는 고정이다:
...
**② 말투 견본 최대 셋.** 블라인드 평가에서 **사용자가 실제로 고른 대사**를
구조화해 둔다(`SELECTED_SAMPLES`). 화자·관계 단계·의도가 맞고, 필요한 사실이
**그 화자에게 투영된 목록에** 전부 있고, 이야기 상태가 그 자리까지 왔을 때만
나간다(`examplesForTurn`). 여기가 지식 차단선이다 — 재언만 아는 과거를 요구하는
견본은 민현 호출의 facts에 그 fact_id가 없어 자동으로 떨어진다.

> **견본은 고정 응답집이 아니다.** 프롬프트가 문장 복사를 명시적으로 금지하고,
> 「견본에 나오는 사건·감정·관계·사실은 지금 입력에 없으면 없는 것」이라고 못박는다.
> 고른 뒤에 견본을 다시 보는 코드는 없다 — 검사도 후처리도 견본을 모른다.
> 테스트가 그 부재를 강제한다. 사용자가 문제로 지목한 문장(「그러던지」·무승자
> 항목·정사를 뒤집는 「공부방을 했어요」)은 안 싣고, 사용자가 직접 고친 문장은
> 수정본만 싣는다.

**③ 고르는 쪽을 말맛 전담으로.** 사실은 앞 단계(hardFilter → Canon → Character)가
이미 걸렀다. 고르는 쪽이 그것을 되짚으면 근거 없는 추측이 판정이 된다.
`SELECTED_DIRECTOR_RULES`는 **코드가 값으로 준 사실과 직접 충돌하는 줄만** 버리고,
나머지는 일곱 축(직접 답했나 · 두 사람이 구별되나 · 자연스러운 한국어인가 ·
구체적인 것에 반응했나 · 관계가 한 칸 움직이나 · 다음 말을 하고 싶나 ·
상담사 말투·질문 도배·이유 없는 시비가 없나)으로만 고른다. 하나만 살아남으면
비교할 상대가 없다고 RETRY하지 않는다.

**안 늘어나는 것.** 일반 턴 호출 수(Writer 1 + Director 1), 캐시 지점 넷,
모델 배치, 검사 단계. selected 장은 cached system 배열의 **비변이 복사** 뒤에
캐시 없이 한 장만 얹는다 — 지점이 늘면 매 턴 캐시가 깨진다. 새 금칙어 정규식은
만들지 않는다: 의도 판정(`intentOf`)은 코드가 이미 아는 것과 **기존 감지기**
(MEMORY_PROBE·FIRSTMEET_ASK·CONFESS_SAY)만 쓴다.
...
첫시도/재시도·**쓴 견본 id**·**쓴 turn material**·route·Effect·자동 검사) ·
attempts.md · report.md · trace/.

**「설레는가 · 캐릭터가 맞는가 · 다음 말을 하고 싶은가」는 자동으로 합격시키지
않는다.** 그 셋은 사람이 answers.md를 읽고 정한다. 자동 검사가 세는 것은
기계가 확실히 아는 실패뿐이다(빈 응답·질문만 함·화자 오류·최종 502).
