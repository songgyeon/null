...
/* ── 두 시계 ──
   ...
   스피드 모드는 쌓인 대화를 날로 센다. 처음에는 네 마디를 하루로 놨다 —
   사다리 숫자가 이미 그 비율로 놓여 있었기 때문이다:
     at   12  26  44  64  90  116
     day   3   7  11  15  20   26      ≒ at ÷ 4
   리얼 모드에서 30일 걸려 닿는 자리에 같은 대화량으로 닿는 게 좋았다.

   그런데 한 마디가 여섯 시간이었다. 네 마디면 하루가 통째로 지나가서,
   한 번 앉아 스무 마디를 나누면 그 사이에 닷새가 흐른다. 대화하는 동안
   며칠이 지나가는 것은 빠른 게 아니라 이상한 것이다. 열여섯으로 올린다 —
   한 마디가 한 시간 반이고, 하루가 예전의 나흘쯤 된다.

   해금은 저절로 따라온다. 사다리가 at(마디 수)과 day(날 수)를 **둘 다**
   보는데, 날이 늦게 가므로 이제 day 쪽이 걸림돌이 된다. 마지막 칸(day 26)에
   닿으려면 116마디가 아니라 416마디다. 판이 그만큼 길어지는 것이 이 값의
   전부다 — 시계만 늦추고 진도는 그대로 두려면 둘을 떼어놔야 하는데,
   그러면 「날 수 = 시각의 경과」라는 등식이 깨진다. 아직은 안 뗀다.

   바뀌는 것은 「실습이 얼마나 진행됐나」뿐이다. 「지금 몇 시인가」는 안 바뀐다 —
   잠도 시간표도 자리 여는 시각도 진짜 시계 그대로다. 세계는 계속 진짜 시간에
   산다. 모드는 판마다 하나고 등록 화면에서 한 번 고른다. 중간에 바꾸면
   D-N이 튄다. */
const SPEED_PER_DAY=16;
...
/* ── 스피드 모드의 시계 ──
   ...
   대화 수를 스스로 볼 수 없다. */
/* 출발 시각이 칸의 자리를 정한다. 일곱 시로 두면 7·13·19·1시를 지나는데
   일곱 시엔 민현이 자고(4:30~8) 한 시엔 재언이 자서 둘이 반쪽이다.
   여덟 시면 네 마디마다 8·14·20·2시에 얹힌다 — 출근·수업·저녁·밤이고,
   자는 사람이 있는 칸은 밤 하나뿐이다. 그 한 칸도 민현은 깨 있다(22~4:30).
   한 마디가 한 시간 반이 된 뒤에도 이 네 자리는 그대로 지나간다. */
